# Layer 06 — Energy Infrastructure MVP Lane Contract
...
- **Canonical Folders:**
  - Fetching: `services/fetch-orchestrator/src/layers/layer_06_energy_infrastructure/`
  - API: `apps/api/src/routes/energy/`
  - Frontend: `apps/web/src/layers/energy/infrastructure/`
  - Tests: `tests/data/layer_06_energy_infrastructure/`
...
|---|-----------|-------------|----------|------------|---------|-----------------|-----------------|-------------------|------------------------|----------------------|
| 1 | `wri_global_power_plant_database` | WRI Global Power Plant Database | https://datasets.wri.org/dataset/globalpowerplantdatabase | Power plants (geolocation, fuel type, capacity, country, owner/operator) | CSV | Creative Commons Attribution 4.0 (CC BY 4.0). Attribution required. | Annual (dataset updated yearly) | Download full CSV to `E:\god-eyes-data\energy\layer_06_energy_infrastructure\wri\raw\global_power_plant_database.csv` | Parse CSV, normalize fuel types, map to canonical categories, extract geometry from lat/lon | - Coverage varies by country (some regions incomplete)<br>- Capacity data may be outdated<br>- Owner/operator fields sometimes missing<br>- No transmission/substation data |
| 2 | `osm_energy_infrastructure` | OpenStreetMap Energy Infrastructure | https://www.openstreetmap.org/ (via Overpass API) | Power lines, substations, generators, pipelines where publicly mapped | XML/JSON (Overpass API) | Open Database License (ODbL). Attribution required. | Manual or periodic Overpass queries | Query Overpass API for energy infrastructure tags, store raw XML/JSON responses per region in `E:\god-eyes-data\energy\layer_06_energy_infrastructure\osm\raw\{region}.json` | Parse OSM XML/JSON, extract nodes/ways/relations with energy tags, map to canonical types, compute geometries | - Coverage depends on OSM volunteer mapping<br>- Data quality varies<br>- Overpass API rate limits<br>- Large regions need chunked queries<br>- No capacity/voltage data unless tagged |
| 3 | `global_energy_monitor_energy` | Global Energy Monitor | https://globalenergymonitor.org/ | Oil/gas pipelines, LNG terminals, major oil/gas terminals | CSV/JSON (if download available) | Creative Commons Attribution 4.0 (CC BY 4.0) for most datasets. Must verify license per dataset before use. | Semi-annual (dataset updates) | Download available datasets to `E:\god-eyes-data\energy\layer_06_energy_infrastructure\gem\raw\` | Parse CSV/JSON, normalize pipeline/terminal attributes, map to canonical types, extract geometries | - License must be verified per dataset<br>- Coverage may be incomplete<br>- Some datasets may require attribution<br>- No real-time flow data<br>- Pipeline routes may be approximate |
...
| `layer_id` | VARCHAR(64) | NO | Always `layer_06_energy_infrastructure` |
...
```json
{
  "features": [
    {
      "id": "uuid",
      "layer_id": "layer_06_energy_infrastructure",
  ...
  "metadata": {
    "layerId": "layer_06_energy_infrastructure",
    "count": 50000,
    "returnedCount": 1000,
    "requestedLimit": 1000,
    "appliedLimit": 1000,
...
- Database migrations in `database/migrations/layers/layer_06_energy_infrastructure/`
- Table definitions with `layer_id` and `source_id`
- PostGIS geometry columns
- Indexes for performance
- Data tests in `tests/data/layer_06_energy_infrastructure/`
...
```sql
CREATE TABLE layer_06_energy_infrastructure.energy_infrastructure (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  layer_id VARCHAR(64) DEFAULT 'layer_06_energy_infrastructure',
...
-- Indexes
CREATE INDEX idx_energy_feature_type ON layer_06_energy_infrastructure.energy_infrastructure (feature_type);
CREATE INDEX idx_energy_category ON layer_06_energy_infrastructure.energy_infrastructure (category);
CREATE INDEX idx_energy_source_id ON layer_06_energy_infrastructure.energy_infrastructure (source_id);
CREATE INDEX idx_energy_country ON layer_06_energy_infrastructure.energy_infrastructure (country);
CREATE INDEX idx_energy_fuel_type ON layer_06_energy_infrastructure.energy_infrastructure (fuel_type);
CREATE INDEX idx_energy_status ON layer_06_energy_infrastructure.energy_infrastructure (status);
CREATE INDEX idx_energy_capacity_mw ON layer_06_energy_infrastructure.energy_infrastructure (capacity_mw);
CREATE INDEX idx_energy_voltage_kv ON layer_06_energy_infrastructure.energy_infrastructure (voltage_kv);
CREATE INDEX idx_energy_geometry ON layer_06_energy_infrastructure.energy_infrastructure USING GiST (geometry);
CREATE INDEX idx_energy_bbox ON layer_06_energy_infrastructure.energy_infrastructure USING GiST (bbox);
```
...
- Fetcher implementations in `services/fetch-orchestrator/src/layers/layer_06_energy_infrastructure/`
- Normalizer implementations in `services/normalizer/src/layers/layer_06_energy_infrastructure/`
- Source manifest
...
```
python -m services.fetch-orchestrator --layer layer_06_energy_infrastructure [options]
...
All raw data stored under: `E:\god-eyes-data\energy\layer_06_energy_infrastructure\`

Structure:
```
E:\god-eyes-data\energy\layer_06_energy_infrastructure\
├── wri\
│   └── raw\
...
- [ ] All required docs exist:
  - [ ] `docs/control/layer_06_energy_infrastructure_mvp_contract.md` (this file)
