...
| 2 | `layer_02_borders_boundaries` | Borders & Boundaries | **coming_soon** | static | No real-time fetchers for MVP. Static GeoJSON source in source catalog. Raw storage optional (can be seeded directly to DB). | Single snapshot table. No history table needed for MVP. | `GET /api/layers/layer_02_borders_boundaries/objects`. Returns GeoJSON feature collection. | Polygon/line overlay on globe. Wireframe or filled with opacity. Must be toggleable independently. | Country borders are politically sensitive. Use authoritative sources (UN, Natural Earth). No disputed territory rendering without explicit spec. |
| 3 | `layer_03_earth_events` | Earth Events | **coming_soon** | live | USGS earthquake API, volcanic activity feeds, weather alerts. Fetchers and normalizers follow standard layer pattern. | Latest snapshot + history/time tables. | `GET /api/layers/layer_03_earth_events/objects`. Support time range queries. | Animated markers (pulsing, color-coded by severity). Timeline scrubber support. | Earthquake/tsunami alerts are time-critical. Cache must be short (< 5 min). Must not cause alert fatigue. |
...
**Author:** Kiro CLI (WO-063-MVP-LAYER-REGISTRY-CONTROL)
