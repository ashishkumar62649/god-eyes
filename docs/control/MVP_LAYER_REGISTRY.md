...
| 3 | `layer_03_earth_events` | Earth Events | **active** | live | USGS earthquake API, volcanic activity feeds, weather alerts. Fetchers and normalizers follow standard layer pattern. | Latest snapshot + history/time tables. | `GET /api/layers/layer_03_earth_events/objects`. Support time range queries. | Animated markers (pulsing, color-coded by severity). Timeline scrubber support. | Earthquake/tsunami alerts are time-critical. Cache must be short (< 5 min). Must not cause alert fatigue. |
