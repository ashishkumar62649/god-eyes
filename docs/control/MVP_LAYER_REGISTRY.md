...
| 7 | `layer_07_weather` | Weather / Live Weather | **coming_soon** | live | Open-Meteo (point/grid weather forecast data). Fetchers in `services/fetch-orchestrator/src/layers/layer_07_weather/`. | Latest observation + history tables. Layer-aware tables with `layer_id`, `source_id`, `location_id`. | `GET /api/layers/layer_07_weather/objects`. Support bbox, temperature, weather_code filters. | Temperature-colored weather markers on globe. Click detail card. Stale data handling. 60 FPS safe. | Weather data is model/grid-based, not street-level exact. Grid-cell resolution varies by model (9-25 km). Coordinate precision vs model resolution must be documented in UI. |
...
| 10 | `layer_10_energy_infrastructure` | Energy Infrastructure | **coming_soon** | static | Public open data sources (WRI, OSM, Global Energy Monitor). Fetchers in `services/fetch-orchestrator/src/layers/layer_10_energy_infrastructure/`. | Single snapshot table. No history for MVP. | `GET /api/energy/infrastructure`. May support category/source filtering. | Point markers for power plants/substations/terminals, line markers for transmission lines/pipelines. Category-based coloring. 60 FPS safe. | Public/open data only. No targeting/sabotage guidance. No real-time operational data. |
...
1. **Globe Core** (`layer_00_globe_core`), **Aviation** (`layer_01_aviation`), **Borders & Boundaries** (`layer_02_borders_boundaries`), and **Earth Events** (`layer_03_earth_events`) have MVP implementations.
2. Layers 04 through 09 must remain visible as **Coming Soon** or **No data yet** until implemented by an approved work order.
...
**Author:** Codex (WO-081A-REPO-GUARDRAILS-LAYER-REGISTRY)
**LLM:** GPT-5.5
