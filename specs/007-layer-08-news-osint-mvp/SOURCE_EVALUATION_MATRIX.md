...
| **GDELT DOC 2.0 API** | None (rate limited) | Global (65 languages) | Near real-time | No (requires geocoding) | Yes (source country, country mentions) | No (but has tone metric) | News discovery, article metadata, trend analysis | High volume, requires location extraction, rate limits | **Priority 2** - News discovery engine |
