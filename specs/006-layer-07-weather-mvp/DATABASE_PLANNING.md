...
`history_id` is generated by ingestion as `sha256("{database_observation_id}|{fetched_at}")[:24]`.
This preserves the deterministic type-aware database observation identity while
allowing the same forecast slot to be captured by multiple fetch runs.

### Ingestion Observation Identity

The normalizer's original `observation_id` is based on
`location_id|source_id|forecast_for`. Current and hourly observations can share
those values, so that ID can collide across observation types. Database ingestion
therefore stores a type-aware ID in the `observation_id` columns:

```python
database_observation_id = sha256(
    f"{location_id}|{source_id}|{observation_type}|{forecast_for}".encode()
).hexdigest()[:24]
```

The original normalizer ID is preserved in
`provider_metadata.logical_observation_id`. History IDs are generated from the
type-aware database observation ID and `fetched_at`. This resolves current/hourly
primary-key collisions without changing normalized source objects or the approved
schema.
...
- If location exists → preserve requested coordinates and update resolved coordinates, elevation, grid metadata, and `updated_at`
