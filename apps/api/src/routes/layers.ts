...
const LAYER_REGISTRY = [
  ...
  },
  {
    ...
    status: 'active' as const,
    dataStatus: 'static' as const,
    description: 'Country borders and administrative boundaries (local/dev via Natural Earth).',
    sourceRule: 'Static GeoJSON source (Natural Earth Admin-0). Seeded to DB. local/dev only; not production boundary-compliant.',
    apiStatus: 'active',
    frontendStatus: 'active',
    safetyNotes: 'Country borders are politically sensitive. local/dev only; not Survey of India compliant; not production-approved. Disputed territories require individual review.',
    isEnabled: true,
    isImplemented: true,
  },
  {
  ...
  },
  {
    ...
    sourceRule: 'Public-only sources for the current build. No classified, no sensitive-source feeds. Static datasets only.',
